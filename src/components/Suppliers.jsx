...
export default function Suppliers({ mode, user }) {
  ...
  // ── Filter logic (defined before handlePrint so filteredSuppliers is available) ──
  ...
  const handlePrint = () => {
    const dataToPrint =
      isSelectionEnabled && selectedIds.length > 0
        ? suppliers.filter((s) => selectedIds.includes(s.supplier_id))
        : filteredSuppliers;
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          ...
          <p class="sub">
            Printed on ${printDate} &nbsp;·&nbsp; ${dataToPrint.length} supplier${dataToPrint.length !== 1 ? "s" : ""}
            ${isSelectionEnabled && selectedIds.length > 0 ? " (selected only)" : ""}
          </p>
          <table>
            <thead>
              <tr>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {/* PRINT — closes menu, opens browser print dialog */}
